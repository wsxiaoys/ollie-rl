...
class TestAppSqlite(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_full_workflow(self):
        ...
        async with httpx.AsyncClient(
            ...
            self.assertEqual(tuner_details["recipe"]["group_size"], 16)
            self.assertEqual(tuner_details["recipe"]["num_groups_per_batch"], 32)
            self.assertNotIn("quarantine_min_samples", tuner_details["recipe"])
            self.assertNotIn("max_succeed_ratio", tuner_details["recipe"])
            self.assertNotIn("max_length_limited_finish_ratio", tuner_details["recipe"])
