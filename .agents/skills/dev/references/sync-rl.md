...
Optional query param `max_expire_rate` (float in `[0, 1]`, omit to disable): when set, the dispenser skips ("quarantines") datums that genuinely keep expiring — those whose recent real-expiration rate is `>= max_expire_rate`. Only expired, unrewarded runs that still have a lingering in-flight op (an `InFlightChatCompletionModel` row — a generation that stalled past the lease) count as real expirations; lost/crashed jobs and runs abandoned after their ops completed are ignored. The rate is measured over a recent policy-generation window so quarantined datums can recover automatically. See the "Expiration quarantine" section in `tuner_service.py` for the full mechanism.
